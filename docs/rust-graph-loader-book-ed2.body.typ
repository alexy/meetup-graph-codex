= Preface
<preface>
This second edition teaches Rust through one concrete implementation:
the By the Bay graph pipeline. The project has two executables and one
shared library module. One executable scrapes and extracts meetup and
conference data. The other executable loads that neutral graph into
several database backends: FalkorDB, HelixDB, and SurrealDB.

The important part is not the databases. The important part is the Rust
design: how data moves through the program, how ownership is preserved,
where borrowing matters, how errors are propagated, how the command line
maps to runtime configuration, and how backend-specific behavior is
hidden behind a stable API.

The first edition focused heavily on ownership and the borrow checker.
Edition 2 keeps that material, but broadens the view to architecture:
module boundaries, data contracts, traits, SDK-vs-HTTP backend
implementations, iterator-heavy functional style, tests, reuse, and the
Rust features that appear in the real code.
...
The library module, `src/graph_loader.rs`, owns database loading. Its
...
This is a good Rust boundary. It is explicit, cheap to call, and
difficult to misuse.
...
Using enums matters. A stringly typed CLI lets invalid values drift deep
into the program. A `ValueEnum` makes invalid values fail at parsing
time.
...
`Clone` and `Copy` are appropriate here because the enum is tiny and has
no heap data. Passing it around by value is simpler than borrowing it.

= 3. Turning CLI Arguments Into Configuration
<turning-cli-arguments-into-configuration>
The CLI struct is specific to the binary. The library uses a separate
config:
...
`Args` owns its `String` fields. `GraphLoadConfig` also needs to own
strings, because it may outlive the local `Args` borrow. Therefore, the
conversion clones strings.

For small configuration strings, cloning is the right tradeoff. It
avoids lifetime complexity and keeps the public config straightforward.

= 4. The Neutral Graph Model
<the-neutral-graph-model>
...
The database-specific node IDs are not part of this model. That keeps
the graph portable across backends.
...
A `HashMap` would also work. `BTreeMap` has one practical advantage
here: deterministic iteration order. That makes generated Cypher,
SurrealQL, and test output more stable.

Determinism is useful in loader code because query generation is easier
to debug when the same input produces the same string order.

= 5. Modeling Property Values
<modeling-property-values>
The loader does not accept arbitrary JSON internally. It uses a small
enum:
...
This is a key design choice. The export data may start as JSON, but the
loader supports only the value types it knows how to write safely across
all backends.
...
<borrowing-for-required-node-ids>
Every node must have a string `id` property. The method that enforces
this is:
...
The node owns the `String` inside `GraphValue::String`. The `id` method
does not clone that string. It returns `&str`, a borrowed view into the
owned string.

The returned reference cannot outlive `self`. Rust enforces that
automatically. That is why this signature is safe:

```rust
fn id(&self) -> Result<&str>
```

The caller can use the borrowed ID while it is still working with the
borrowed node, but cannot stash it somewhere longer-lived without
copying it.
...
The loader is an application-level tool, not a reusable low-level crate.
It uses `anyhow::Result`:
...
<reading-the-consolidated-export>
...
+ `fs::read_to_string` returns an owned `String`.
+ `serde_json::from_str` borrows that string while parsing.
+ `GraphExport` owns the parsed data.
+ `to_graph_data` borrows `GraphExport` and creates owned `GraphData`.
...
These structs are private to the binary. That is a good boundary: the
JSON format is an input concern, not a graph-loading concern.

= 9. Converting Export Records Into Nodes
<converting-export-records-into-nodes>
...
Why clone? Because the graph node must own its property strings. The
source `MeetupGroup` is only borrowed. Moving fields out of a borrowed
struct is not allowed, and should not be allowed: the caller still owns
the export.
...
It turns `Option<String>` borrowed through `&MeetupGroup` into
`Option<&str>`. That matches the helper:

```rust
fn insert_optional(props: &mut BTreeMap<String, GraphValue>, key: &str, value: Option<&str>)
```

This avoids cloning optional strings unless they are actually present
and non-empty.

= 10. Building Maps With Const Generics
<building-maps-with-const-generics>
...
`const N: usize` lets the function accept arrays of any length while
preserving static type information.
...
Without const generics, you would likely accept a slice or vector. The
array version is compact at call sites and allocation-free before the
final map is collected.

= 11. Optional Properties and Mutable Borrows
<optional-properties-and-mutable-borrows>
...
That mutable borrow means the caller temporarily gives the helper
exclusive access to the map. While the helper is running, no other code
can read or write the same map. After the function returns, the borrow
ends and the caller can use the map again.
...
<alternate-importer-per-talk-records>
...
`serde(alias = "...")` is useful when ingesting adjacent formats. It
lets the Rust code keep one field name, `label`, while accepting JSON
that says `type` or `kind`.
...
<moving-values-into-deduplication-maps>
...
The map key needs owned strings, so the key clones the fields. If the
edge is not already present, `or_insert_with` moves the original strings
into the stored `GraphEdge`.

Why not avoid the clones? You could restructure this code to clone less,
but this version is simple and correct. For a loader dominated by
database I/O, the small string clones are not the bottleneck.

Rust makes the cost visible, which lets you choose intentionally.

= 14. Converting JSON Values Safely
<converting-json-values-safely>
...
This is boundary normalization. Inside the loader, backends do not need
to deal with arbitrary JSON. They only handle `String` and
`StringArray`.

The function takes `serde_json::Value` by value, not by reference. That
means it can move strings out of the JSON without cloning:

```rust
serde_json::Value::String(value) => GraphValue::String(value)
```

If it took `&serde_json::Value`, this branch would need to clone the
string.
...
The outside world does not need to know how backend dispatch works. It
only needs `load_graph`. Keeping the trait private gives us freedom to
change the internal design without breaking callers.

= 16. Why Not Return Trait Objects?
<why-not-return-trait-objects>
...
That is useful when the selected backend must be stored and reused as a
runtime object. This loader does not need that. It dispatches once,
performs the load, and exits.
...
Rust rewards choosing the simplest abstraction that fits the lifetime of
the problem.

= 17. Backend-Specific Ownership
<backend-specific-ownership>
Each backend borrows the neutral graph and generates backend-specific
commands.
...
- `connection` is borrowed mutably because Redis commands mutate
  connection state.

That last point is a common Rust pattern. Network clients and database
connections often require `&mut self` because sending a command changes
buffers, sequence state, or protocol state.
...
`String::with_capacity(value.len() + 2)` is a small performance hint.
The output will be at least two bytes longer because of the surrounding
quotes. Escapes may make it longer, but this capacity avoids
reallocating for the common case.

= 19. Backend Differences Shape the Code
<backend-differences-shape-the-code>
...
The abstraction works because all three can honor the same portable
contract:
...
<where-helix-affects-the-property-model>
Live Helix rejected array-valued properties in graph mutations. That
directly affects the backend writer:
...
SurrealDB can preserve arrays; Helix currently cannot through this
mutation path.
...
No nodes are copied. The batch function only sees a borrowed window over
the existing vector.

`config.batch_size.max(1)` prevents a panic. A chunk size of zero is
invalid, so the loader clamps it to at least one.
...
This is pragmatic for a command-line loader. The binary can stay simple
while individual backends use async where required.

For a long-running server, you would probably make the public API async
instead of creating runtimes internally.

= 23. Lifetimes Without Writing Lifetimes
<lifetimes-without-writing-lifetimes>
Most of this project does not write explicit lifetime parameters. Rust
infers them.
...
You usually write explicit lifetimes only when a function returns a
borrowed value and there is more than one possible input lifetime. This
code mostly has obvious borrowing relationships, so elision keeps it
readable.

= 24. Tests as Design Locks
<tests-as-design-locks>
...
This test is not just checking syntax. It captures a live backend
constraint: Helix should not receive array properties.
...
<practical-borrow-checker-rules-from-this-project>
...
<how-the-abstractions-were-chosen>
...
That abstraction exists because the scraper should not know how
FalkorDB, HelixDB, or SurrealDB write graph data.
...
That abstraction exists because the CLI needs to select a backend at
runtime.
...
That abstraction exists because backend implementations should have a
common shape while keeping the public API stable.

A useful rule: abstractions should protect a real boundary. Here the
real boundaries are:
...
The current implementation is intentionally practical. Good next steps
would be:
...
- split backend modules into separate files once the single module
  becomes too large

Do not split files just to split files. Split when navigation,
ownership, or testability improves.

= Conclusion
<conclusion>
This graph loader is a useful Rust learning project because it is small
enough to understand and real enough to show the language's strengths.
...
The most important Rust idea here is ownership at boundaries. Parse into
owned data. Borrow while loading. Clone only when a second owner is
actually needed. Move values when consuming an input format. Keep
backend-specific details behind a stable interface.
...
allowed to leak into each other. The scraper can improve its parsing
rules without caring whether the graph is later written to FalkorDB,
HelixDB, or SurrealDB. The database loader can add a backend without
knowing how Meetup pages were scraped.
...
Each stage consumes data that has become stable enough for that layer,
then emits a simpler value for the next layer.

= 29. Crate Layout and Module Boundaries
<crate-layout-and-module-boundaries>
The crate is named `bythebay-scraper`, but it exposes a reusable library
module:
...
That one-line `src/lib.rs` is small, but it changes the architecture. It
lets the `load_graph` binary use the loader through the crate boundary:
...
This avoids copying loader code into the binary. It also makes public
and private API explicit. `GraphData`, `GraphNode`, `GraphEdge`,
`GraphValue`, `GraphBackend`, `GraphLoadConfig`, and `load_graph` are
public because they are the loader contract. The individual backend
structs and helper functions are private because callers should not
...
That short `main` is a strong signal. The binary parses arguments, reads
input, and delegates. The interesting loader behavior lives in the
library, not in the executable wrapper.

= 31. Data Contracts: Raw, Export, Graph
<data-contracts-raw-export-graph>
The project uses several data shapes because one shape would be too
vague:
...
`RawMeetupEvent`, and `RawMeetupSession` reflect scraper concerns. They
contain optional fields because real web data is incomplete.
...
In `src/graph_loader.rs`, `GraphData` removes domain-specific record
names:
...
`serde(alias = "...")` accepts alternate input field names.
`serde(default)` makes missing fields explicit. The project uses dynamic
`serde_json::Value` at the uncertain edge, then converts into typed
structs when the shape is known.

= 33. Enums as Closed Worlds
<enums-as-closed-worlds>
...
This gives the compiler a complete list of backend choices. If a new
backend is added, the match in `load_graph` points at every dispatch
...
The loader intentionally supports only values that all target backends
can handle predictably. The enum is a portable property contract.

= 34. Traits as Backend Ports
<traits-as-backend-ports>
...
This is a port-and-adapter design in compact Rust form. The trait names
the port: "load this graph." The structs are adapters: "load it into
Falkor," "load it into Helix through HTTP," "load it into SurrealDB
through the SDK."

The trait is private because there is no need for callers to implement
their own loaders yet. Keeping it private gives the project freedom to
change the backend internals without breaking the public API.

= 35. SDK Backends Versus HTTP Backends
<sdk-backends-versus-http-backends>
HelixDB and SurrealDB each have two loader paths.

The HTTP variants build request payloads or query strings directly, then
post them through `reqwest::blocking::Client`.

The Rust SDK variants use client libraries and SDK-specific builders,
then run async operations inside a Tokio runtime created by the
synchronous loader.
...
The HTTP path is transparent because you can inspect the exact JSON
payload. The SDK path is more structured because Rust types and builder
methods catch some mistakes before the request is sent.
...
That means the same user-facing
`--surreal-url http://127.0.0.1:8000/sql` is adapted for the SDK. The
helper `surreal_ws_address` exists because the SDK and HTTP interfaces
do not use identical endpoint shapes.

= 36. What Got Reused and Abstracted
<what-got-reused-and-abstracted>
The SDK loaders do not duplicate the whole backend implementation. They
reuse the same neutral graph model, config, batching policy, and
transformation helpers.

Helix HTTP and Helix SDK both use `relationship_type`, `helix_base_url`,
graph chunks from `graph.nodes.chunks(config.batch_size.max(1))`, and
the same omission of `labels` and string arrays from Helix properties.
They differ in only the final representation: HTTP builds
`serde_json::Value`\; the SDK builds `write_batch()` DSL requests.

SurrealDB HTTP and SurrealDB SDK both use `surreal_bootstrap_query`,
`surreal_delete_tables_query`, `surreal_create_nodes_query`,
`surreal_create_edges_query`, `surreal_id_tables`, `surreal_table_name`,
and `surreal_string`. They differ in transport and session management:
HTTP sends Basic auth and namespace/database headers per request; the
SDK signs in once, selects namespace/database, and sends queries through
the SDK.

The code abstracts backend choice, graph property values, repeated query
construction, and repeated data-cleaning operations. It reuses data
contracts and semantic helpers first. It abstracts execution strategies
only when they really have the same shape.

= 37. Functional Programming in the Codebase
<functional-programming-in-the-codebase>
The project uses a practical Rust version of functional programming:
iterators, maps, filters, folds-by-collection, and expression-oriented
transforms.
...
This pipeline says: borrow each URL, keep only the first occurrence,
transform it into a `GraphNode`, and collect the result.
...
`filter_map` combines validation and transformation. Invalid array
entries disappear. Valid strings move into the output vector.

Rust's iterator style is not merely cosmetic. It controls ownership.
`iter()` borrows. `into_iter()` consumes. `cloned()` makes owned copies
at visible points.

= 38. Option as a Data-Cleaning Tool
<option-as-a-data-cleaning-tool>
The project uses `Option<T>` for missing data, but also as a small
functional pipeline:
...
This helper reuses one rule everywhere: only insert optional string
properties when they exist and are not empty. The function takes
`Option<&str>`, not `Option<String>`, because callers often hold
borrowed data:
...
`map` transforms the value if it exists. `and_then` chains a
transformation that may itself reject the value.

= 39. Result and Error Context
<result-and-error-context>
The project uses `anyhow::Result` for application-level errors. This is
appropriate because the binaries need clear failure messages more than
they need a stable typed error API.
...
preserves the original error and adds local meaning. The loader also
uses `bail!` when the program detects a semantic problem, such as a
graph node without a string `id`.

= 40. Ownership: Moves, Clones, and Reuse
<ownership-moves-clones-and-reuse>
...
`Args` and `GraphLoadConfig` both own their strings. That makes the
config easy to pass around without borrowing from the CLI parser's local
value.
...
`into_values` consumes it and moves each `GraphNode` into the output
vector.
...
<small-helper-functions-as-policy-reuse>
...
optional-property filtering. `relationship_type` centralizes
backend-safe relationship labels. `surreal_string` delegates string
escaping to `serde_json::to_string`, avoiding hand-written quote
escaping.

`surreal_table_name`, `surreal_identifier`, and `relationship_type`
centralize name normalization rules so every backend query does not
invent its own rules. `post_helix`, `post_surrealdb`, and
`post_surrealdb_sdk` centralize response checking. This is not just code
reuse. It is policy reuse: what counts as a failed backend write lives
in one helper per transport.

= 42. Async Rust in a Mostly Synchronous Loader
<async-rust-in-a-mostly-synchronous-loader>
The scraper is naturally async because it fetches remote resources. It
uses Tokio and async `reqwest`:

```rust
async fn fetch(client: &Client, url: &str) -> Result<String>
```

The loader binary is mostly synchronous because file reading,
command-line parsing, and Falkor/HTTP loading are simple blocking
operations.

The SDK backends introduce async APIs into that synchronous loader. The
code bridges the two worlds by creating a runtime:
...
This keeps the public `GraphLoader` trait synchronous. If the project
later needs high-concurrency loading, it could make the trait async or
split sync and async loaders. Today, the synchronous trait is simpler.

= 43. Generics in the Project
<generics-in-the-project>
...
This lets callers pass arrays of any fixed length without
heap-allocating a temporary vector.
...
The practical meaning is: "give me any type that serde can deserialize
from this response text."
...
Rust pattern matching appears everywhere in the project. The most
visible examples are `match` expressions over enums and JSON values.
...
That reads as: if this optional value exists, bind it; otherwise
continue the loop. It is clearer than a nested `if let` when the rest of
the loop requires the value.
...
For talk-record import, this key deduplicates edges by source, target,
and relationship:
...
Determinism matters for generated files, test expectations, and database
query debugging. A graph loader that emits stable output is easier to
reason about.

= 46. Backend-Specific Semantics
<backend-specific-semantics>
FalkorDB accepts Cypher-like strings through Redis commands. Its loader
creates one query per node and one query per edge. Labels are encoded
directly in the Cypher pattern, and properties are written with
`SET n +=`.

HelixDB HTTP accepts a dynamic-query JSON format. Nodes and edges become
a list of named query steps plus named returns.

HelixDB SDK accepts typed DSL expressions. The code builds a write
batch, names intermediate variables with `var_as`, and returns selected
variables.

SurrealDB accepts SurrealQL. Nodes are created with table/id pairs, and
relationships are created with `RELATE`.

These databases do not share one query language. The architecture shares
the portable graph and isolates each query language behind a backend
adapter.

= 47. Property Model Differences Across Backends
<property-model-differences-across-backends>
The property model is where backend differences become visible.

FalkorDB can receive strings and arrays in generated Cypher, so
`cypher_map` serializes both `GraphValue::String` and
`GraphValue::StringArray`.

SurrealDB can receive strings and arrays in SurrealQL assignments, so
`surreal_node_props` writes both.

Helix currently receives only string properties from this loader. Both
Helix paths omit string arrays:
...
`StringArray`, `tags_csv` as a string, and `tags_json` as a string.
Backends with array support can use `tags`. Backends with string-only
property support still receive useful tag data.

= 48. Testing as Executable Design Notes
<testing-as-executable-design-notes>
...
variables correctly, SurrealDB creates complete sample graph statements,
replace queries include known labels, and URL conversion works for SDK
endpoints.

The import tests assert that export records and talk-record files both
become the same neutral `GraphData` shape.

These tests are design locks. They protect the small transformations
that would be painful to debug only after a database write failed.

= 49. Key Rust Features Referenced by the Codebase
<key-rust-features-referenced-by-the-codebase>
...
- `impl` blocks for constructors, conversions, and backend
  implementations
- `derive` macros for `Debug`, `Clone`, `Default`, `Deserialize`,
  `Serialize`, and `ValueEnum`
- attributes such as `#[arg(...)]`, `#[serde(...)]`, `#[tokio::main]`,
  and `#[cfg(test)]`
- owned types such as `String`, `PathBuf`, `Vec<T>`, `BTreeMap<K, V>`,
  and `BTreeSet<T>`
- borrowed types such as `&str`, `&PathBuf`, `&GraphData`, and slices
  like `&[GraphNode]`
- `Option<T>` for missing data
- `Result<T>` and `?` for fallible code
- iterators: `iter`, `into_iter`, `map`, `filter`, `filter_map`,
  `flat_map`, `collect`
...
The valuable point is not that the project uses many features. The
valuable point is that each feature has a job. Rust is strongest when
types, ownership, and module boundaries describe the architecture
directly.

= 50. Edition 2 Summary
<edition-2-summary>
The first edition used the loader to explain the borrow checker. Edition
2 shows the wider design:
...
The result is not a giant framework. It is a practical Rust application
whose architecture is visible in its types.
