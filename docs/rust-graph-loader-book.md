...
`scripts/enrich_meetup_entities.py`. That file uses the same top-level `nodes`
and `edges` shape, but its node labels include `Speaker`, `Company`, and
`Project` in addition to talks, meetups, and conferences.
